...
if _HAS_TRITON:
    ...
    def _flash_fwd_kernel(
        ...
        q = tl.load(
            q_ptr + pid_bh * Q_LEN * HEAD_DIM + offs_m[:, None] * HEAD_DIM + offs_d[None, :],
        ...
        for n_start in range(0, KV_LEN, BLOCK_N):
            k = tl.load(
                k_ptr + pid_bh * KV_LEN * HEAD_DIM + (n_start + offs_n)[:, None] * HEAD_DIM + offs_d[None, :],
            ...
            if IS_CAUSAL:
                mask = mask & ((n_start + offs_n)[None, :] <= offs_m[:, None])
            ...
            v = tl.load(
                v_ptr + pid_bh * KV_LEN * HEAD_DIM + (n_start + offs_n)[:, None] * HEAD_DIM + offs_d[None, :],
                mask=((n_start + offs_n)[:, None] < KV_LEN) & (offs_d[None, :] < HEAD_DIM),
                other=0.0,
            ).to(tl.float32)
        ...
        acc = acc / safe_l[:, None]
        tl.store(
            o_ptr + pid_bh * Q_LEN * HEAD_DIM + offs_m[:, None] * HEAD_DIM + offs_d[None, :],
            acc,
            mask=valid[:, None] & (offs_d[None, :] < HEAD_DIM),
        )
        tl.store(
            lse_ptr + pid_bh * Q_LEN + offs_m,
            row_m + tl.log(safe_l),
            mask=valid,
        )
    ...
    def _flash_bwd_kernel(
        ...
        do = tl.load(
            do_ptr + pid_bh * Q_LEN * HEAD_DIM + offs_m[:, None] * HEAD_DIM + offs_d[None, :],
            mask=(offs_m[:, None] < Q_LEN) & (offs_d[None, :] < HEAD_DIM),
            other=0.0,
        ).to(tl.float32)
        lse = tl.load(
        ...
        for n_start in range(0, KV_LEN, BLOCK_N):
            ...
            tl.atomic_add(
                dv_ptr + pid_bh * KV_LEN * HEAD_DIM + (n_start + offs_n)[:, None] * HEAD_DIM + offs_d[None, :],
                dv,
                sem="relaxed",
                mask=((n_start + offs_n)[:, None] < KV_LEN) & (offs_d[None, :] < HEAD_DIM),
            )
            tl.atomic_add(
                dk_ptr + pid_bh * KV_LEN * HEAD_DIM + (n_start + offs_n)[:, None] * HEAD_DIM + offs_d[None, :],
                dk,
                sem="relaxed",
                mask=((n_start + offs_n)[:, None] < KV_LEN) & (offs_d[None, :] < HEAD_DIM),
            )
        ...
        tl.store(
            dq_ptr + pid_bh * Q_LEN * HEAD_DIM + offs_m[:, None] * HEAD_DIM + offs_d[None, :],
            dq,
            mask=(offs_m[:, None] < Q_LEN) & (offs_d[None, :] < HEAD_DIM),
        )
else:
...
def _validate_triton_flash_inputs(q, k, v, attn_bias, dropout_p, is_causal):
    ...
    require(q.shape[:2] == k.shape[:2] == v.shape[:2], "backend='flash' requires matching batch/head dims")
    ...
    require(not is_causal or q.size(-2) == k.size(-2), (
        "backend='flash' causal mode currently requires q_len == kv_len"
    ))
...
class _TritonFlashAttention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, is_causal):
        ...
        lse = torch.empty(B, H, q_len, device=q.device, dtype=torch.float32)
        grid = (triton.cdiv(q_len, _TRITON_BLOCK_M), B * H)
...
def triton_flash_attention(q, k, v, attn_bias=None, dropout_p=0.0, is_causal=False):
    _validate_triton_flash_inputs(q, k, v, attn_bias, dropout_p, is_causal)
    return _TritonFlashAttention.apply(q, k, v, bool(is_causal))
