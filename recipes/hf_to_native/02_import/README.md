...
from scratch in the local-training recipes, so the later alignment
recipes can use it without HF-specific code paths.

```bash
bash recipes/hf_to_native/02_import/run.sh
```

By default the recipe loads `smollm2-135m`, writes the converted
...
step is where the import is checked: it forwards both the original
...
smaller mean the mapping is clean, and the residual is fp32 accumulation
order.
...
source logits. Gemma3 is rejected by the model-type guard. That guard matters because the
