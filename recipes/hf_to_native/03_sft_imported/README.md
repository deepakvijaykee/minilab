...
recipes, with only the input checkpoint and the learning rate
differing. Because SFT runs through the native code path rather than
the Hugging Face trainer, the imported baseline can be compared directly
with a from-scratch checkpoint under matched alignment code, which is
the motivation for the import path in the first place.
...
`--batch-size 1`, `--lr 2e-5`, and `--max-examples 500`. Output is
written to `checkpoints/imported/smollm2-135m-sft`.
