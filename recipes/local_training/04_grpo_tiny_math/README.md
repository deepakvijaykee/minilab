...
SFT checkpoint from recipe 02 (`sft/step_500` by default, overridable
with `SFT_CHECKPOINT`), samples short rollouts for each problem, scores
each rollout against the GSM8K numeric answer with the deterministic
...
The defaults are conservative because RLVR memory cost
...
whenever all rollouts in the group tie. At two generations most
...
adding more steps at group size two and is the first parameter to raise
when the run produces no learning signal.
